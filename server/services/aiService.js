...
function isPlaceholderKey(value) {
  return /^(YOUR_[A-Z0-9_]+|YOUR_API_KEY_HERE|CHANGE_ME|REPLACE_ME|sk-.*PLACEHOLDER.*)$/i.test(value);
}

function getProviderConfig() {
  const key = process.env.OPENAI_API_KEY && process.env.OPENAI_API_KEY.trim();
  if (!key) {
    throw new Error('AI features are disabled because no valid API key is set in server/.env. Add your real key and restart the backend.');
  }
  if (isPlaceholderKey(key)) {
    throw new Error('The current OpenAI/Groq key in server/.env is only a placeholder. Replace it with your real API key and restart the backend.');
  }
  ...
  return { client, model, isOpenRouter, isGroq };
}
...
const transcribeAudio = async (buffer, mimetype = 'audio/webm') => {
  ...
  const { client, isOpenRouter, isGroq } = getProviderConfig();
  ...
  try {
    const result = await client.audio.transcriptions.create({
      file,
      model: isGroq ? (process.env.WHISPER_MODEL || 'whisper-large-v3') : 'whisper-1',
      language: 'en',
    });
